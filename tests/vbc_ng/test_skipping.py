...
def test_skip_av1_logic():
    config = AppConfig(general=GeneralConfig(threads=1, skip_av1=True))
    
    # File already AV1
    vf_av1 = VideoFile(
        path=Path("already_av1.mp4"),
        size_bytes=1000,
        metadata=VideoMetadata(width=1920, height=1080, codec="av1", fps=30)
    )
...
def test_min_compression_ratio_revert(tmp_path):
    ...
    config = AppConfig(general=GeneralConfig(threads=1, min_compression_ratio=0.1))
    ...
    mock_exif.extract_metadata.return_value = VideoMetadata(width=100, height=100, codec="h264", fps=30)
