...
def test_orchestrator_sequential_flow():
    ...
    # Setup test data
    root_dir = Path("/tmp/videos")
    test_file = VideoFile(path=root_dir / "test.mp4", size_bytes=1000)
    mock_file_scanner.scan.return_value = [test_file]
    
    # Mock metadata extraction
    mock_exif.extract_metadata.return_value = VideoMetadata(width=1920, height=1080, codec="h264", fps=30.0, camera_model="Sony")
    ...
    # 2. Metadata
    mock_exif.extract_metadata.assert_called_once_with(test_file)
    ...
    assert job_arg.source_file == test_file
