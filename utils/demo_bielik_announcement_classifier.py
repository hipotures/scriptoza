...
DEFAULT_PREPARE_CSV = "semantic_bielik_windows.csv"
DEFAULT_PREPARE_JSONL = "semantic_bielik_windows.jsonl"
DEFAULT_CLASSIFY_JSONL = "semantic_bielik_classification.jsonl"
DEFAULT_CLASSIFY_CSV = "semantic_bielik_classification.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare and classify transcript windows for Bielik/OpenAI-compatible announcement experiments."
    )
    ...
    classify_parser.add_argument(
        ...
        default="codex-exec",
        help="Classification backend. Default: codex-exec",
    )
    classify_parser.add_argument(
        "--api-base-url",
    ...
    classify_parser.add_argument(
        "--no-json-mode",
        action="store_true",
        help="Disable response_format json_object in the chat completion request.",
    )
...
def build_prompt(window: Dict) -> str:
    ...
    return (
        ...
        "- Prefer explicit phrases like 'numer startowy 279' over category ordinals like 'szosty w tej kategorii'.\n"
...
def extract_json_payload(text: str) -> Dict:
    ...
    if stripped.count("[") > stripped.count("]"):
        stripped = stripped + ("]" * (stripped.count("[") - stripped.count("]")))
    if stripped.count("{") > stripped.count("}"):
        stripped = stripped + ("}" * (stripped.count("{") - stripped.count("}")))
...
def classify_windows(args: argparse.Namespace) -> int:
    ...
    if not args.model:
        ...
        return 1
    if args.backend == "openai-compatible" and not args.api_base_url:
        ...
        while pending_windows:
            ...
            if args.backend == "codex-exec" and len(batch) > 1:
                ...
                try:
                    ...
                    response_payload = extract_json_payload(raw_response_text)
